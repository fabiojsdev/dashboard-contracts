...
import { CreditCard, CheckCircle, Clock, AlertTriangle, Pencil } from 'lucide-react'
import { moeda, dataFmt } from '../utils/formatters'
import { diaVencimento, condInquilino, calcTaxaAdm, calcRepasse, totalInquilino, MESES, contratoAtivoNoMes, today } from '../utils/calculos'
...
export default function Pagamentos() {
  ...
  function getStatus(contrato) {
    ...
    const venc = new Date(ano, mes-1, diaVencimento(contrato.dataAssinatura))
  ...
  function salvarPagamento() {
    const payload = { contratoId:alvoContrato.id, mes, ano, status:'pago', dataPagamento:form.dataPagamento, obs:form.obs }
    if (editing) dispatch({ type:'UPDATE_PAGAMENTO', payload:{ ...payload, id:editing } })
    else         dispatch({ type:'ADD_PAGAMENTO',    payload })
    setModal(false)
  }

  function marcarPendente(contratoId) {
    const pg = getPag(contratoId)
    if (pg) dispatch({ type:'UPDATE_PAGAMENTO', payload:{ ...pg, status:'pendente', dataPagamento:null } })
  }
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{MESES[mes]} / {ano}</p>
        </div>
      ...
      <div className="stats-grid">
        {[
          { label:'Recebido', value:moeda(totalRecebido), color:'#2d9e6b' },
          { label:'Aluguel Bruto', value:moeda(totalBruto), color:'var(--primary)' },
          { label:'Taxa Adm (6%)', value:moeda(totalTaxa), color:'var(--accent)' },
          { label:'Repasses', value:moeda(totalRepasse), color:'#5b6dc8' },
        ].map(c => (
          <div key={c.label} className="stat-card" style={{ padding:'14px 16px' }}>
            ...
            <div style={{ fontSize:isMobile?18:20, fontWeight:700, color:c.color, fontFamily:"'Playfair Display'" }}>{c.value}</div>
          </div>
        ))}
      </div>
      ...
      {contratosAtivos.length === 0 ? (
        <div className="card">
          <div className="empty-state"><CreditCard size={40}/><p>Nenhum contrato ativo neste mês</p></div>
        </div>
      ) : isMobile ? (
        // ── Mobile: Cards ──
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {contratosAtivos.map(c => {
            ...
            return (
              <div key={c.id} className="card" style={{ padding:'14px 16px', borderLeft: status==='atrasado' ? '3px solid var(--danger)' : status==='pago' ? '3px solid var(--success)' : '3px solid var(--warning)' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                  <div>
                    <div style={{ fontWeight:700, fontSize:15 }}>{inquilino?.nome ?? '–'}</div>
                    <div style={{ fontSize:12, color:'var(--text-muted)' }}>{imovel?.endereco?.split(',')[0]}</div>
                  </div>
                  {STATUS_BADGE[status]}
                </div>

                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'6px 12px', fontSize:13 }}>
                  <InfoRow label="Vencimento" value={`Dia ${dia}`} bold />
                  <InfoRow label="Total Inquilino" value={moeda(totalInq)} bold color="var(--primary)" />
                  <InfoRow label="Aluguel" value={moeda(c.valorAluguel)} />
                  <InfoRow label="Condomínio" value={enc ? moeda(condInq) : '⚠ sem encargo'} color={!enc?'var(--warning)':undefined} />
                  <InfoRow label="Taxa Adm" value={moeda(taxa)} color="var(--accent)" />
                  <InfoRow label="Repasse Prop." value={moeda(repasse)} color="#2d9e6b" />
                  {pg?.dataPagamento && <InfoRow label="Pago em" value={dataFmt(pg.dataPagamento)} />}
                </div>

                <div style={{ display:'flex', gap:8, marginTop:12 }}>
                  {status !== 'pago' ? (
                    <button className="btn btn-primary btn-sm" style={{ flex:1 }} onClick={() => openMarcarPago(c)}>
                      <CheckCircle size={13}/> Marcar como Pago
                    </button>
                  ) : (
                    <>
                      <button className="btn btn-ghost btn-sm" onClick={() => marcarPendente(c.id)}>Reverter</button>
                      <button className="btn btn-ghost btn-sm" onClick={() => openMarcarPago(c)}><Pencil size={13}/> Editar</button>
                    </>
                  )}
                </div>
              </div>
        ...
        <div className="table-wrap">
          <table>
            <thead>
              <tr><th>Inquilino</th><th>Imóvel</th><th>Vence</th><th>Aluguel</th><th>Cond.</th><th>Total Inquilino</th><th>Taxa Adm</th><th>Repasse</th><th>Pgto</th><th>Status</th><th>Ação</th></tr>
            </thead>
            <tbody>
              {contratosAtivos.map(c => {
                const inquilino = state.inquilinos.find(i => i.id === c.inquilinoId)
                ...
                return (
                  <tr key={c.id} style={{ background: status==='atrasado' ? 'rgba(192,57,43,0.03)' : undefined }}>
                    ...
                    <td style={{ fontSize:12, color:'var(--text-muted)' }}>{imovel?.endereco?.split(',')[0] ?? '–'}</td>
                    <td style={{ fontWeight:700 }}>Dia {dia}</td>
                    <td style={{ fontWeight:700 }}>{moeda(c.valorAluguel)}</td>
                    <td>{enc ? moeda(condInq) : <span className="badge badge-warning" style={{fontSize:10}}>Sem encargo</span>}</td>
                    ...
                    <td style={{ fontSize:12 }}>{pg?.dataPagamento ? dataFmt(pg.dataPagamento) : '–'}</td>
                    <td>{STATUS_BADGE[status] || STATUS_BADGE.pendente}</td>
                    <td>
                      <div style={{ display:'flex', gap:4 }}>
                        {status !== 'pago' ? (
                          ...
                          <>
                            <button className="btn-icon" onClick={() => openMarcarPago(c)} title="Editar"><Pencil size={14}/></button>
                            <button className="btn btn-ghost btn-sm" onClick={() => marcarPendente(c.id)}>Reverter</button>
                          </>
                        )}
                      </div>
      ...
      {modal && alvoContrato && (
        <Modal title="Registrar Pagamento" onClose={() => setModal(false)}
          footer={<><button className="btn btn-ghost" onClick={() => setModal(false)}>Cancelar</button><button className="btn btn-primary" onClick={salvarPagamento}><CheckCircle size={14}/> Confirmar</button></>}>
          <div className="info-box">
            <strong>{state.inquilinos.find(i => i.id === alvoContrato.inquilinoId)?.nome}</strong>
            {' · '}{MESES[mes]}/{ano}
          </div>
          <Grid2>
            <Input label="Data do pagamento" type="date" value={form.dataPagamento} onChange={set('dataPagamento')} />
          </Grid2>
          <Textarea label="Observações" value={form.obs} onChange={set('obs')} placeholder="Ex: Pago via PIX…" />
        </Modal>
      )}
    </div>
  )
